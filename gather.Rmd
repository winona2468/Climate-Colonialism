...
# What countries count as colonized? This is the first question because the data set could be split many ways... I will run the RMSE of each in my model to determine which is best. This information will go on later Shiny tabs, but I am including it as the first part of this .RMD so my data sets are created and defined!
...
# Above I am calculating averages by continent instead of by colonial status.
 ...
 select(Name, status, world_region, avg_risk) %>%
 slice(1) %>%
 ggplot(mapping = aes(x = world_region, y = avg_risk)) + 
...
# Choosing Model 1 because it has the lowest SD. L
...
print(model_emissions_stan, digits = 4)
...
``` {r Tab 3 Effects of Empire Plots}

# PLOT 1: ONSET AND END OF COLONIALISM
...
onset_end_plot <- colonialism %>%
  ggplot(aes(y = country_name, x = , color = colonizer)) +
  geom_line()
